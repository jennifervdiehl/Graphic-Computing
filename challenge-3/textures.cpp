...
//Códdigo fonte do Fragment Shader (em GLSL): ainda hardcoded
...
int main()
{
    ...
    auto obj = parse_obj("/Users/i587264/Documents/Personal/University/PG_CG/Graphic-Computing/challenge-3/objects/suzanneTri.obj");
    if (!obj) {
...
GLFWwindow* initializeGL()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Diga olá ao Suzanne - Jennifer Diehl!", nullptr, nullptr);
    if (!window)
    {
...
int setupTexture(const std::string& filepath, GLenum min_filter, GLenum mag_filter)
{
    ...
    if (!data) {
        cerr << "Failed to load texture path " << filepath; abort();
    }
    
    if (!(channels == 4 || channels == 3)) {
        cerr << "num channels not supported, texture: " << filepath; abort();
    }
